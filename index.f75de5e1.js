...
function app() {
    ...
    table.addEventListener("dblclick", (e)=>{
        const cell = e.target;
        if (cell.tagName === "TD") cellEdit(cell);
    });
    selectUser();
...
function cellEdit(cell) {
    ...
    input.addEventListener("keypress", (e)=>{
        if (e.key === "Enter") saveChanges(input.value, cell, prevValue);
    });
    const saveChanges = (value, cellToChange, prev)=>{
        if (value) {
            cellToChange.innerHTML = value;
            input.remove();
        } else cellToChange.innerHTML = prev;
    };
}
function createForm() {
    const datas = [
...
function sortLogic(headers, tbody) {
    ...
    const sortColumn = (indexOfColumn, isAscending)=>{
        ...
        rows.sort((a, b)=>{
            ...
            let result;
            if (!isNaN(aValue) && !isNaN(bValue)) result = aValue - bValue;
            else result = aValue.localeCompare(bValue);
...
function addUser(e) {
    ...
    if (user.name.length < 4 || !user.name.trim().length) {
        createNotification("error", "Fix your name", "The name must consist of at least 4 letters.");
        return;
    }
    if (!user.position.trim().length) {
        createNotification("error", "Fix your position", "Position field can not be empty.");
        return;
    }
    if (user.office === null) {
        createNotification("error", "Fix your office", "Ofice field can not be empty.");
        return;
    }
    ...
    if (!parseFloat(user.salary.replace(/[$,]/g, ""))) {
        createNotification("error", "Fix your salary", "Salary field can not be empty");
        return;
    }
